...
def test(apk_file1,apk_file2):
    dex_result = check_repack(apk_file1,apk_file2)
    sign_result = compare_signature(apk_file1,apk_file2)
    if not sign_result:
        print "签名检测结果：是重打包软件"
    else:
        print "签名检测结果：不是是重打包软件"
    if dex_result:
        print "dex检测结果：是重打包软件"
    else:
        print "dex检测结果：不是是重打包软件"

if __name__ == '__main__':
    apkpath='/Users/sundiz/Desktop/repack.uu.pltm.apk'
    apkpath1='/Users/sundiz/Desktop/uu.pltm.apk'
    test(apkpath, apkpath1)
    pass
